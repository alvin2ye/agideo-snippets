...
namespace :backup do
  ...
  task :crontab do
    puts <<-CRONTAB
      m h d m d command
      0 3 * * * cd /var/www/do8 && /usr/bin/rake backup:log RAILS_ENV=production > /dev/null 2>&1
      5 3 * * * cd /var/www/do8 && /usr/bin/rake backup:db:mysql RAILS_ENV=production > /dev/null 2>&1
    CRONTAB
  end

  desc 'rake backup:log RAILS_ENV=production'
  task :log => :environment do
    ...
    # 执行备份命令
    cmd = <<-CMD
      request-log-analyzer log/#{Rails.env}.log > log/analyse.txt
      tar -czvf #{filename} log/#{Rails.env}.log log/analyse.txt
    CMD
    `#{cmd}`
